...
void TowerPanleLayer::addTower(int type){
	static_cast<BaseMap*>(this->getParent()->getParent())->playerState->removeTowerInfo();//预览信息移除
	switch (type){
	case(2):{
...
void TowerPanleLayer::inAnimation(){
	...
	planeSprite->runAction(Sequence::create(ScaleTo::create(0.1f,1.0f,1.0f),
		CallFuncN::create(CC_CALLBACK_0(TowerPanleLayer::addIcons,this)),NULL));
}

void TowerPanleLayer::addTempTower(int type){
	if(tempTower != NULL) removeChild(tempTower,true);
