...
void ::GameOption::initButton(){
	//右上角关闭按钮
	auto close = MenuItemSprite::create(
		Sprite::createWithSpriteFrameName("options_close_0001.png"),
		Sprite::createWithSpriteFrameName("options_close_0002.png"));
	close->setCallback([&](Ref *pSender){
		SoundManager::playClickEffect();
		static_cast<PlayerStateMenu*>(this->getParent())->removeGameOption();//调用父类PlayerStateMenu方法来移除当前层恢复游戏
	});
	...
	effect->setTag(2);
	if(UserDefault::getInstance()->getIntegerForKey("backeffect",1) == 0){//0表示禁止音效，1表示开启
		addOff(effect);
	}
