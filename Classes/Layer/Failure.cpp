...
bool Failure::init(){
	...
	option->runAction(Sequence::create(
			...
			,CallFuncN::create(CC_CALLBACK_0(Failure::pauseGame, this)),NULL));
...
void Failure::initMap(){
	...
	auto defeatLabel = Label::createWithTTF("DEFEAT!", "ObelixPro.ttf", 30);//defeat标题
	...
	auto items = Sprite::createWithSpriteFrameName("defeat_items.png");//包含宝石、道具等的图片
...
void Failure::onTouchEnded(Touch* touch, Event* event){
	...
	case(0)://重新开始
		Director::getInstance()->resume();//先恢复游戏
