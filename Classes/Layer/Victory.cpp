...
bool Victory::initVictory(int star,int gem){
    ...
    auto color = LayerColor::create(Color4B(0, 0, 0, 60), visibleSize.width, visibleSize.height);//黑色半透明的背景遮罩
    addChild(color,-1);
	...
	auto Sequence = Sequence::create(CallFuncN::create(CC_CALLBACK_0(Victory::VSpriteAnimation, this)),
								     ...
								     CallFuncN::create(CC_CALLBACK_0(Victory::showGems, this)),NULL);
	runAction(Sequence);
...
void Victory::VSpriteAnimation(){VictorySprite->runAction(ScaleTo::create(0.5f,1.0f,1.0f));}
...
void Victory::OnContinueTouch(Ref* pSpender){
	Director::getInstance()->replaceScene(TransitionGame::create(1.0f, GameView::createScene()));//游戏过渡动画，一秒后回到选择关卡界面
}

void Victory::OnRestartTouch(Ref* pSpender){
	Director::getInstance()->replaceScene(TransitionGame::create(1.0f, GameScene::playGame(level,difficult)));//一秒后重新开始当前游戏
}