...
bool PlayerStateMenu::init(){
	...
	backPackSpritelistener->onTouchBegan = [&](Touch* touch, Event* event){
		...
		if (rect.containsPoint(locationInNode) && backPackSprite->isVisible()) return true;
		return false;
	};
	...
	for(int i = 0; i < 6 ; i ++){
		int num = UserDefault::getInstance()->getIntegerForKey(__String::createWithFormat(instance->SLOTX_SHOP,i)->getCString());//读取道具数量
		if(num > 0)stringForSkillFileName = __String::createWithFormat("backPack_icons_000%d.png",i+1)->getCString();
		else stringForSkillFileName = __String::createWithFormat("backPack_icons_off_000%d.png",i+1)->getCString();//根据道具的有无显示不同图标
		...
		backPack_icons_Sprite[i]->setName(__String::createWithFormat(instance->SLOTX_SHOP,i)->getCString());//name存储Key，方便道具数量的扣除
		...
		numLeft->setTag(101);//设置tag用于后续获取道具数量的更新
	...
	pause_listener->onTouchEnded = [&](Touch* touch, Event* event){
		...
		removeTowerInfo();
		removeMonsterInfo();
		addGameOption();//暂停前要关闭背包并移除所有信息面板
	};
		...
		if(rect.containsPoint(locationInNode)){
			if(completeThunder == true){
				...
				if(thunderStoneSprite->getName() == "inactive"){
					//初次点击，设置为点击状态，将背包与援兵按钮设置为未点击状态并关闭背包，将触摸监听器设置为火雨的监听器
	...
	skillListener->onTouchBegan = [&](Touch* touch, Event* event){
		...
		if (rect.containsPoint(locationInNode) && backPackSprite->isVisible()){
			...
			if(num <= 0){
				...
				static_cast<Label*>(target->getChildByTag(101))->setString("0");//数量标签设置为0
	...
	for(int i = 0; i < 6; i++) _eventDispatcher->addEventListenerWithSceneGraphPriority(skillListener->clone(),backPack_icons_Sprite[i]);
...
void PlayerStateMenu::initGameOption(){
	gameOption = GameOption::create();//实例创建

	gameOption->level = getLevel();
	gameOption->difficult = getDifficult();//参数传入

	gameOption->setPosition(Point(0,Director::getInstance() -> getVisibleSize().height));
	addChild(gameOption,99);
}

void PlayerStateMenu::addGameOption(){
	auto color = LayerColor::create(Color4B(0, 0, 0, 60), Director::getInstance() -> getVisibleSize().width,
									Director::getInstance() -> getVisibleSize().height);
	color->setTag(501);
	addChild(color);

	auto gameOptionListener = EventListenerTouchOneByOne::create();
	gameOptionListener->onTouchBegan = [&](Touch* touch, Event* event){return true;};
...
void PlayerStateMenu::removeGameOption(){
	...
	gameOption->runAction(MoveBy::create(0.2f,Point(0,Director::getInstance() -> getVisibleSize().height)));//向上移出屏幕外
...
void PlayerStateMenu::startProgressTimers(){
	this->schedule(schedule_selector(PlayerStateMenu::updateStoneProgress));//根据实际情况来更新进度.
	this->schedule(schedule_selector(PlayerStateMenu::updateParatrooperProgress));//根据实际情况来更新进度.
}
...
void PlayerStateMenu::clearMonsters(){
	...
	for(int i = 0;i<monstersVector.size();i++){
		...
		if(monster != NULL) monster->explosion();
	}
}
...
void PlayerStateMenu::shopSkill(int type){
	...
	case(0):{
		...
		atomicBomb_plane->runAction(Sequence::create(
			...
			CallFuncN::create(CC_CALLBACK_0(Sprite::removeFromParent, atomicBomb_plane)),NULL));//飞机飞过，投下核弹
