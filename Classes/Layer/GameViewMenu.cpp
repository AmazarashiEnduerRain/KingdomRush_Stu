...
bool GameViewMenu::init(){
	...
	shop = MenuItemSprite::create(
		Sprite::createWithSpriteFrameName("butShop_0001.png"),
		Sprite::createWithSpriteFrameName("butShop_0002.png"));
	update = MenuItemSprite::create(
		Sprite::createWithSpriteFrameName("butUpgrades_0001.png"),
		Sprite::createWithSpriteFrameName("butUpgrades_0002.png"));
	book = MenuItemSprite::create(
		Sprite::createWithSpriteFrameName("butEncyclopedia_0001.png"),
		Sprite::createWithSpriteFrameName("butEncyclopedia_0002.png"));

	update->setCallback([&](Ref *pSender){
		SoundManager::playClickEffect();
		Director::getInstance()->getInstance()->pushScene(Upgrades::createScene());//进行场景堆叠。关卡选择界面的场景仍然存在
	});
	...
	mapStarsContainer->setPosition(Point(winSize.width - mapStarsContainer->getContentSize().width / 2 - 10,
										 winSize.height - mapStarsContainer->getContentSize().height / 2 + 60));
