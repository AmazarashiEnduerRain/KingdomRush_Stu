...
bool LoadingScene::init(){
	...
	auto back = Sprite::create("Default@2x.png");//加载背景图。原图竖屏，旋转
...
void LoadingScene::loadSouce(){
	...
	numberOfLoadedRes++;//完成第一阶段音效与音乐加载
...
void LoadingScene::logic(float dt){
	if(numberOfLoadedRes == 0) loadSouce();

    if (numberOfLoadedRes == 2){//加载完成时，直接切换至欢迎界面
