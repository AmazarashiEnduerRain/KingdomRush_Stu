...
bool BeginAnimation::init(){
	...
	listener->setSwallowTouches(true);//吞噬触摸事件，防止玩家的点击穿透动画层
...
void BeginAnimation::startAnimation1(){
     ...
     sp1->setPosition(Point(0,size.height*0.5));
     addChild(sp1,1);
	...
	listener->onTouchBegan = [&](Touch* touch, Event* event){
		...
		Rect rect = Rect(-size,width / 2, -size.height / 2, size.width / 2, size.height / 2);
	...
	listener->onTouchEnded = [&](Touch* touch, Event* event){_eventDispatcher->removeEventListener(listener);};
...
void BeginAnimation::startAnimation2(){
     ...
     sp11->setPosition(Point(size.width * 0.33,size.height * 1.0));
     addChild(sp11,2);
