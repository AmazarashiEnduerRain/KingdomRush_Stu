...
void BaseArtilleryTower::shoot(float dt){
	...
	if(isReady && nearestMonster!=NULL && nearestMonster->getCurrHp() > 0){
		...
		runAction(Sequence::create(CallFuncN::create(CC_CALLBACK_0(BaseArtilleryTower::fireAnimation,this)),
			CallFuncN::create(CC_CALLBACK_0(BaseArtilleryTower::fire,this,firePosition)),NULL));
	}
}
...
void BaseArtilleryTower::fire(Point firePosition){
	...
	float endRotate;
	if(shootVector.x > currBullet->getPosition().x) endRotate = 180.0f;
	else endRotate = -180.0f;
	...
	currBullet->shoot();
	runAction(Sequence::create(DelayTime::create(1.0f),CallFuncN::create(CC_CALLBACK_0(BaseArtilleryTower::filledAnimation,this)),NULL));
	currBullet = NULL;
}
...
Bullet* BaseArtilleryTower::ArtilleryTowerBullet(){
	auto bullet = Bomb::createBombWithType(getLevel());//根据塔等级创建不同炸弹外观
	bullet->setPosition(Point(0,30));
	this->getParent()->addChild(bullet);//添加到地图层
    return bullet;
}
