...
bool MechsTower::init(){
	...
	layer3->runAction(Sequence::create(
		...
		Animate::create(AnimationCache::getInstance()->getAnimation("MechsTower_layer3_r")),NULL));
	layer4->runAction(Sequence::create(
		DelayTime::create(2.0f),
		Animate::create(AnimationCache::getInstance()->getAnimation("MechsTower_layer4_r")),
		CallFuncN::create(CC_CALLBACK_0(MechsTower::playReadySound,this)),NULL));
...
void MechsTower::playReadySound(){ SoundManager::playMechReady();}

void MechsTower::initMech(){
	mecha = Mecha::create();
	mecha->setPosition(Point(0,40));
	this->getParent()->addChild(mecha);//机甲不作为塔的子节点，从而可以任意移动
}
...
void MechsTower::setRallyPoint(Point location){ mecha->runToLocation(location - this->getParent()->getPosition());}