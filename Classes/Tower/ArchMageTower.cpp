...
void ArchMageTower::sellTower(){
	for(int i = 0;i < 3;i++){
		if(DelayBullet[i] != NULL) DelayBullet[i]->removeFromParent();
	}
...
void ArchMageTower::buildDelayBullet(){
	for(int i = 0;i < 3;i++){
		if(DelayBullet[i] == NULL){
			...
			MoveTo* move;
			if(i == 1) move = MoveTo::create(1.5f,Point(20 * i - 20,75));
			else move = MoveTo::create(1.5f,Point(20 * i - 20,65));
...
void ArchMageTower::Attack(float dt){
	...
	checkNearestMonster();
	if(nearestMonster == NULL) buildDelayBullet();
