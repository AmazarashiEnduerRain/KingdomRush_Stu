	...
	if(nearestMonster != NULL && nearestMonster->getCurrHp() > 0){
		...
		if( attackCount != 3){
			...
			HitDecal->runAction(Sequence::create(
				DelayTime::create(1.0f),CallFuncN::create(CC_CALLBACK_0(Sprite::setVisible,HitDecal,true)),
				ScaleTo::create(0.5f,3.5f,3.5f),CallFuncN::create(CC_CALLBACK_0(Sprite::setVisible,HitDecal,false)),NULL));
			...
			glow->runAction(Sequence::create(
				...
				CallFuncN::create(CC_CALLBACK_0(Sprite::setVisible,glow,false)),NULL));
			attackCount = 0;
		}
	}else{
		...
		if(shouldDrill){
			...
			light->runAction(Repeat::create(Animate::create(AnimationCache::getInstance()->getAnimation("EarthquakeTower_light")),10));

			operation->runAction(Sequence::create(
			Animate::create(AnimationCache::getInstance()->getAnimation("EarthquakeTower_drilling")),
			CallFuncN::create(CC_CALLBACK_0(Drill::shoot,drill)),NULL));
		}
	}
}
