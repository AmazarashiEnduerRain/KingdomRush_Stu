...
bool TotemTower::init(){
	if(!BaseArrowTower::init()) return false;
...
void TotemTower::TotemCD(float dt){ totemCD = true;}
...
void TotemTower::checkNearestMonster(){
	...
	for(int i = 0; i < monsterVector.size(); i++){
		...
		if (distance < scope_ && monster->getAttackByTower()) {
			currMinDistant = distance < currMinDistant ? distance : currMinDistant;
            monsterTemp = monster;
			monstersCount++;
		}
	}
