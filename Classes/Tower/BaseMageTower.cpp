...
Bullet* BaseMageTower::MageTowerBullet(){ return NULL;}
...
void BaseMageTower::shoot(float dt){
	...
	if(nearestMonster!=NULL && nearestMonster->getCurrHp() > 0)
    {
		...
		currBullet->setRotation(180.0f - angleDegrees);//角度修正
		...
		if(shootVector.y > 0) shooter->runAction(Animate::create(AnimationCache::getInstance()->getAnimation("mage_shoot_up")));
		else shooter->runAction(Animate::create(AnimationCache::getInstance()->getAnimation("mage_shoot_down")));
