...
bool Twister::init(){
	...
	addChild(sprite);
	nearestMonster = NULL;
	scheduleOnce(schedule_selector(Twister::removeTwisterAction), 5.0f);//总持续时间五秒
	return true;
}
...
void Twister::round(){
	sprite->runAction(Sequence::create(
		...
		CallFuncN::create(CC_CALLBACK_0(Twister::attackMonster,this)),NULL));
}
...
void Twister::attackMonster(){
	...
	sprite->runAction(
		...
		schedule(schedule_selector(Twister::hitMonster), 0.5f);
		schedule(schedule_selector(Twister::move), 1.0f);//每0.5s造成伤害；每1s进行移动
}
...
void Twister::removeTwisterAction(float dt){
	...
	sprite->runAction(Sequence::create(
		...
		CallFuncN::create(CC_CALLBACK_0(Twister::removeTwister, this)),NULL));
}
...
void Twister::removeTwister(){ this->removeFromParent();}
