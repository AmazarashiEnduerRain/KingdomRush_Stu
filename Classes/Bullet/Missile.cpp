...
void Missile::shoot(BaseMonster* monster,Point mechaPosition){
    ...
    if(monster->getHpBar()->getPercentage() < 20.0f){
        ...
        for (int j = 0; j < monsterVector.size(); j++){
            if(monster != NULL && monsterVector.at(j)->getHpBar()->getPercentage() > 20.0f && (monster->getAttackByTower())){
    ...
    auto shootVector = monster->baseSprite->getPosition() - this->getParent()->getPosition() - mechaPosition;//飞行向量
    auto pVectr = shootVector;
    ...
    sprite->runAction(Sequence::create(
        ...
        CallFuncN::create(CC_CALLBACK_0(Missile::removeBullet, this)),NULL));
}

void Missile::removeBullet(){
    auto monster = monsterToBeDrilled;
    ...
    sprite->runAction(Sequence::create(
        ...
        CallFuncN::create(CC_CALLBACK_0(Missile::removeFromParent, this)),NULL));
}