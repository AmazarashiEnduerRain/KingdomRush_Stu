...
void ArchMageBolt::shoot(){
    runAction(Sequence::create(bulletAction,CallFuncN::create(CC_CALLBACK_0(ArchMageBolt::removeBullet, this)),NULL));
}
...
void ArchMageBolt::removeBullet(){
    ...
    auto bulletRect = Rect(this->getPositionX() + this->getParent()->getPositionX() - this->getContentSize().width / 2,
        ...
        this->sprite->getContentSize().width,this->sprite->getContentSize().height );
