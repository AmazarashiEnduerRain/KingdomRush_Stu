...
bool MapFlag::initWithLevel(int level){
	...
	switch (UserDefault::getInstance()->getIntegerForKey(__String::createWithFormat(instance->LEVELX_STARNUM, getLevel())->getCString(), 0)){
	case(0): //得星个数为0，未完成
		setType(0);
...
void MapFlag::onTouchEnded(Touch* touch, Event* event){
	Director::getInstance()->pushScene(ChooseDifficulty::createSceneWithLevel(getLevel()));//触摸结束，场景转换
	switch (type){//重设旗帜
	case(1):
...
bool MapFlag::onTouchBegan(Touch* touch, Event* event){
	...
	Point locationInNode = target->convertTouchToNodeSpace(touch);//获取被触摸的精灵并进行坐标转换
...
void MapFlag::addStar(){
	if (type != 0){
		...
		int shiningNum = UserDefault::getInstance()->getIntegerForKey(__String::createWithFormat(instance->LEVELX_STARNUM, getLevel())->getCString(), 0);
		if (shiningNum > 0){
...
void MapFlag::starAnimation(){
	...
	int shiningNum = UserDefault::getInstance()->getIntegerForKey(__String::createWithFormat(instance->LEVELX_STARNUM, getLevel())->getCString(), 0);
	if (shiningNum > 0){
		for (int i = 0; i <= shiningNum - 1; i++) setStarShine(i);
	}
}

void MapFlag::setStarShine(int num){
	starVector.at(num)->runAction(Sequence::create(DelayTime::create(num * 0.3),
	Animate::create(AnimationCache::getInstance()->getAnimation("shine")),NULL));
}
