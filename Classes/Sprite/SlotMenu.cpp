...
bool SlotMenu::initWithNum(int num){
	...
	setNum(num);
	savelot = Sprite::createWithSpriteFrameName("mainmenu_saveslot_0001.png");//槽的背景
	addChild(savelot);
	...
	button_Savelot_Delete = MenuItemSprite::create(Sprite::createWithSpriteFrameName("mainmenu_saveslot_delete_0001.png"),
			...
			CC_CALLBACK_1(SlotMenu::button_Savelot_Delete_callback, this));//MenuItemSprite，点击时调用回调函数
	button_Savelot_Delete->setPosition(Point(savelot->getContentSize().width / 2 - 20,savelot->getContentSize().height / 2 - 20));
	auto menu = Menu::create(button_Savelot_Delete,NULL);//容器
	...
	if(UserDefault::getInstance()->getIntegerForKey(__String::createWithFormat("Slot%d",getNum())->getCString(),0)==0){
		...
		auto label_slot = Label::createWithTTF(__String::createWithFormat("SLOT %d",num)->getCString(),
			"Comic_Book.ttf",32,Size::ZERO,TextHAlignment::LEFT,TextVAlignment::TOP);
		...
		auto slot = Label::createWithTTF(__String::createWithFormat("%d/65",starCountInt)->getCString(),
			"Comic_Book.ttf",32,Size::ZERO,TextHAlignment::LEFT,TextVAlignment::TOP);//星星数量的标签
