...
void MonsterInfoLayer::updateMonsterHp(float dt){
	...
	else hpLabel->setString(__String::createWithFormat("%d/%d",(int)mBaseMonster->getCurrHp(),(int)mBaseMonster->getMaxHp())->getCString());
}
...
void MonsterInfoLayer::setMonsterInfo(){
	...
	info_bg->addChild(protraits);//头像框背景
	...
	info_bg->addChild(monsterName);//怪物名
	Sprite* portraits_sc;
	...
	case(SCORPION):
		...
		monsterName->setString("Scorpion");
		break;
	case(WASPHORNET):
