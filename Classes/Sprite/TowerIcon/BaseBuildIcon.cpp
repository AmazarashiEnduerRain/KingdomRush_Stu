...
bool BaseBuildIcon::initIcon(int type,int money){
	...
	priceLabel->setColor(Color3B(255,153,0));//Red = 255, Green = 153, Blue = 0
...
void BaseBuildIcon::setNotEnoughMoney(){
	isAble = false;
	priceLabel->setColor(Color3B(102,102,102));//价格文字设为灰色；图标也切换为相应金钱不足版本
...
void BaseBuildIcon::update(float dt){
	if(getMoney() <= GameManager::getInstance()->MONEY) setEnoughMoney();
	else setNotEnoughMoney();
}