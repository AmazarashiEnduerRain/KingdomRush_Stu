...
BaseMonster::BaseMonster():baseSprite(nullptr),pointCounter(0),
runSpeed(0),maxHp(0),currHp(0),armor(0),hpPercentage(100)
,hpBar(NULL),lastState(stateNone),attackBySoldier(true)
,attackByTower(true),isAttacking(false),isSlowed(false)
,money(0)
{}
...
void BaseMonster::setMonsterZorder(int yOrder)
{
    ...
    switch (hunder)
    {
        case(0):this->setLocalZOrder(10);break;
        case(1):this->setLocalZOrder(9);break;
        case(2):this->setLocalZOrder(8);break;
        case(3):this->setLocalZOrder(7);break;
        case(4):this->setLocalZOrder(6);break;
        case(5):this->setLocalZOrder(5);break;
        case(6):this->setLocalZOrder(4);break;
        case(7):this->setLocalZOrder(3);break;
        case(8):this->setLocalZOrder(2);break;
        case(9):this->setLocalZOrder(1);break;
        case(10):this->setLocalZOrder(0);break;
        default:break;
    }
}
...
void BaseMonster::update(float dt)
{
    ...
    if(lastState != getState()){
        ...
        case(stateWalkLeft):{
            ...
            baseSprite->runAction(action);}break;
        case(stateWalkUp):{
        ...
        case(stateAttackLeft):{
            ...
            baseSprite->runAction(action);}break;
        case(stateNone):{lastState = stateNone;}break;
        case(stateFrozen):{lastState = stateFrozen;}break;
        case(stateDeath):{lastState = stateDeath;}break;
    }
}

void BaseMonster::stopMonsterAnimation(){
    for(int i = 1 ;i <= 5;i++) baseSprite->stopActionByTag(i);
}
...
void BaseMonster::slowRunSpeed(float dt,float dis)
{
    if(!isSlowed){
        ...
        CallFuncN::create(CC_CALLBACK_0(BaseMonster::setIsSlowed, this,false)),NULL));
    }
}
...
bool BaseMonster::onTouchBegan(Touch* touch, Event* event){
    ...
    if (rect.containsPoint(locationInNode) && this->isVisible()) return true;
    return false;
}
