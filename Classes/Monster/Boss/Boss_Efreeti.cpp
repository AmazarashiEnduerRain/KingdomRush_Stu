...
bool Boss_Efreeti::init(){
	...
	cloud->setPosition(Point(baseSprite->getContentSize().width / 2,baseSprite->getContentSize().height / 2));//确保云雾特效始终正好处于身体正下方
...
void Boss_Efreeti::skill(float dt){
	...
	baseSprite->runAction(Sequence::create(
		...
		CallFuncN::create(CC_CALLBACK_0(Boss_Efreeti::setIsAttacking, this,false)),NULL));
}
