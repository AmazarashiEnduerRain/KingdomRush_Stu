...
//移动地图层来达到移动镜头的效果
void Level5::beforeBoss(){this->runAction(MoveTo::create(0.5f,Point(0,Director::getInstance()->getVisibleSize().height - 1000)));}
...
void Level5::waveEvent(){
	if(wave == 1){
		...
		for (int i = 1; i <= 36; i++){
			...
			if(frame1 != nullptr) animFrames1.pushBack(frame1);
		}
		...
		door->runAction(Sequence::create(
			...
			CallFuncN::create(CC_CALLBACK_0(Level5::stonesEffect,this)),NULL));
		schedule(schedule_selector(Level5::addMonsters), 1.0f, waveVector.at(wave).size(), 6.0f);
	}else schedule(schedule_selector(Level5::addMonsters), 1.0f, waveVector.at(wave).size(), 0.0f);
}
