...
void BaseMap::initMap(){
	isStart = false;//游戏是否开始的标记
	isEnd = false;//最后一波是否出完的标记
	GameManager::getInstance()->LEVEL = level;//在全局游戏管理器中记录当前关卡
	...
	wave = -1;//设置初始波数为-1.第一波为0
	time = 0;//清零计时器

	addTerrains();
	initTouchLayer();
	setMapPosition();//根据地图大小调整摄像机初始位置
}
...
void BaseMap::showWaveProgressBars(float dt){
	for (int i = 0; i < waveFlags.size(); i++) waveFlags.at(i)->restartWaveFlag();
}
		...
		waveflag_listener->onTouchBegan = [&](Touch* touch, Event* event){
			...
			if (rect.containsPoint(locationInNode) && target->isShown){
				if (target->selected->isVisible()){
					for (int i = 0; i < waveFlags.size(); i++) waveFlags.at(i)->stopRespiration();
				}
				for (int i = 0; i < waveFlags.size(); i++) waveFlags.at(i)->setSelected();
				return true;
			}
			return false;
		};
...
void BaseMap::victory(){
	...
	{
		case(0):
			if (GameManager::getInstance()->LIFE >= 18) gotStar = 3;
			else if (GameManager::getInstance()->LIFE > 10) gotStar = 2;
			else gotStar = 1;
			break;
		case(1):
			gotStar = 4;
...
void BaseMap::addWaves(float dt){
	...
	}//检查是否有任何一个“下一波”旗帜被点击或计时结束

	if (newWave && wave < maxWave){
		isStart = true;
		SoundManager::playIncomingWave();
		wave ++;
		for (int i = 0; i < waveFlags.size(); i++) waveFlags.at(i)->setWavePercentage(0.0f);
