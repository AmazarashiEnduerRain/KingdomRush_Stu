...
void Level4::addOrnament(){
	...
	for(int i = 0;i < 4;i++){
		...
		treeTerrains[i]->setPosition(Point(985 + i * 10,530));
	}

	for(int i = 3;i >= 0;i--) mapSprite->addChild(treeTerrains[i],0);
	//将树加入地图
	for(int i = 0;i < 4;i++) for(int j = 0;j < 3;j++) TreeSprite[i][j] = NULL;
...
void Level4::waveEvent(){
	...
	}else schedule(schedule_selector(Level4::addMonsters), 1.0f, waveVector.at(wave).size(), 0);
}
