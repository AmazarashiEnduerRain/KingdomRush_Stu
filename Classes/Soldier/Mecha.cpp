...
bool Mecha::init(){
	if (!BaseSoldier::init()) return false;
...
void Mecha::shitCD(float dt){couldShit = true;}
...
void Mecha::shootMissiles(){
	...
	baseSprite->runAction(Sequence::create(Animate::create(AnimationCache::getInstance()->getAnimation("Mecha_shoot")),
		...
		CallFuncN::create(CC_CALLBACK_0(Missile::shoot,missile,farestMonster1,this->getPosition())),NULL));
}
...
void Mecha::bombMonster(float dt){
	...
	if(couldShit && monsterCount >= 3){
		...
		if(nearestMonster != NULL && nearestMonster->getCurrHp() > 0){
			...
			currBullet->shoot();
			baseSprite->runAction(Sequence::create(Animate::create(AnimationCache::getInstance()->getAnimation("Mecha_attack")),NULL));
			currBullet = NULL;
		}else{
			checkFarestMonster();
			if(farestMonster1 != NULL && farestMonster1->getCurrHp() > 0){
				...
				shootMissiles();
			}else baseSprite->setFlippedX(!(baseSprite->isFlippedX()));//反转朝向，待机动作
		}
	}
}
...
void Mecha::shitting(){ shit->runAction(Animate::create(AnimationCache::getInstance()->getAnimation("Mecha_shitting")));}
...
void Mecha::runToLocation(Point point){
	...
	runAction(Sequence::create(
		...
		CallFuncN::create(CC_CALLBACK_0(Sprite::stopAllActions,baseSprite)),NULL));
...
void Mecha::checkFarestMonster(){
	...
	for(int i = 0; i < monsterVector.size(); i++){
		...
		if (distance > currMaxDistant && monster->getAttackByTower()) {
			currMaxDistant = distance;
            monsterTemp = monster;
		}
	}
